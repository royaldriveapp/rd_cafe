...
const MenuItemCard = memo(({ item, variants, onClick }: MenuItemCardProps) => {
  ...
  const frontContent = hasImage ? (
    ...
    <div className="relative w-full h-full rounded-2xl overflow-hidden border border-border/40 bg-gradient-to-t from-secondary to-card shadow-soft flex flex-col justify-end p-4">
      {hasVideo && (
        <div className="absolute top-3 right-3 bg-background/80 backdrop-blur-sm rounded-full p-1.5 opacity-70 group-hover:opacity-100 transition-opacity">
          <Play size={14} className="text-foreground fill-foreground" />
        </div>
  ...
  // Back face content — nutritional info
  const backContent = (
    <div
      className="w-full h-full rounded-2xl overflow-hidden border border-border/40 bg-gradient-to-br from-card via-secondary/60 to-card shadow-soft flex flex-col justify-between p-5 cursor-pointer"
    ...
    >
      <div>
        ...
        {item.calories !== undefined && (
          <div className="inline-flex items-center gap-1.5 bg-primary/10 text-primary rounded-full px-2.5 py-1 mb-3">
            <Flame size={12} />
  ...
  return (
    <motion.div
      ...
      className="group"
    >
      <div className="flip-card aspect-[4/3]" onClick={handleFlip}>
