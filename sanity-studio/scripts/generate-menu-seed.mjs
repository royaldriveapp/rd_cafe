import fs from "node:fs";
import path from "node:path";
import { fileURLToPath } from "node:url";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);
const studioDir = path.resolve(__dirname, "..");
const appDir = path.resolve(studioDir, "..");
const outputDir = path.join(studioDir, "seed-data");
const outputPath = path.join(outputDir, "menu.ndjson");

const menuItems = [
  {
    _id: "menu-item-1",
    _type: "menuItem",
    name: "Signature Latte",
    description: "Smooth espresso with silky steamed milk and artisan latte art",
    price: "$5.50",
    category: "coffee",
    imagePath: "src/assets/coffee-latte.jpg",
    videoUrl: "https://www.youtube.com/watch?v=fN_iiDCxjPo",
    ingredients: ["Espresso", "Steamed Milk", "Vanilla Syrup"],
    calories: 220,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-2",
    _type: "menuItem",
    name: "Classic Espresso",
    description: "Rich, bold, and perfectly extracted single origin beans",
    price: "$4.00",
    category: "coffee",
    imagePath: "src/assets/espresso.jpg",
    videoUrl: "https://www.youtube.com/watch?v=fN_iiDCxjPo",
    ingredients: ["Single Origin Coffee Beans", "Filtered Water"],
    calories: 5,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-3",
    _type: "menuItem",
    name: "Cappuccino",
    description: "Perfect balance of espresso, steamed milk, and velvety foam",
    price: "$5.00",
    category: "coffee",
    ingredients: ["Espresso", "Steamed Milk", "Milk Foam"],
    calories: 150,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-4",
    _type: "menuItem",
    name: "Flat White",
    description: "Double ristretto with microfoam for a creamy finish",
    price: "$5.25",
    category: "coffee",
    ingredients: ["Double Ristretto", "Microfoam Milk"],
    calories: 140,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-5",
    _type: "menuItem",
    name: "Pour Over",
    description: "Hand-crafted single origin coffee, brewed to perfection",
    price: "$6.00",
    category: "coffee",
    ingredients: ["Single Origin Beans", "Filtered Water"],
    calories: 5,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-6",
    _type: "menuItem",
    name: "Cold Brew",
    description: "Smooth, naturally sweet, steeped for 18 hours",
    price: "$5.50",
    category: "coffee",
    ingredients: ["Coarse Ground Coffee", "Cold Filtered Water"],
    calories: 10,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-7",
    _type: "menuItem",
    name: "Mocha",
    description: "Espresso meets rich chocolate and steamed milk",
    price: "$6.00",
    category: "coffee",
    ingredients: ["Espresso", "Dark Chocolate", "Steamed Milk", "Whipped Cream"],
    calories: 310,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-8",
    _type: "menuItem",
    name: "Affogato",
    description: "Creamy vanilla gelato drowned in hot espresso",
    price: "$7.00",
    category: "coffee",
    ingredients: ["Vanilla Gelato", "Espresso"],
    calories: 280,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-9",
    _type: "menuItem",
    name: "Matcha Latte",
    description: "Ceremonial grade matcha with oat milk",
    price: "$6.00",
    category: "beverages",
    ingredients: ["Ceremonial Matcha", "Oat Milk", "Honey"],
    calories: 180,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-10",
    _type: "menuItem",
    name: "Chai Latte",
    description: "Spiced chai with warming cinnamon and cardamom",
    price: "$5.50",
    category: "beverages",
    ingredients: ["Black Tea", "Cinnamon", "Cardamom", "Ginger", "Steamed Milk"],
    calories: 200,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-11",
    _type: "menuItem",
    name: "Hot Chocolate",
    description: "Rich Belgian chocolate with whipped cream",
    price: "$5.00",
    category: "beverages",
    ingredients: ["Belgian Chocolate", "Whole Milk", "Whipped Cream"],
    calories: 350,
    dietaryTags: ["Vegetarian", "Contains Dairy"],
  },
  {
    _id: "menu-item-12",
    _type: "menuItem",
    name: "Fresh Juice",
    description: "Seasonal fruits pressed to order",
    price: "$6.50",
    category: "beverages",
    ingredients: ["Seasonal Fruits", "Ice"],
    calories: 120,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-13",
    _type: "menuItem",
    name: "Sparkling Water",
    description: "San Pellegrino 500ml",
    price: "$4.00",
    category: "beverages",
    ingredients: ["Carbonated Mineral Water"],
    calories: 0,
    dietaryTags: ["Vegan", "Gluten-Free"],
  },
  {
    _id: "menu-item-14",
    _type: "menuItem",
    name: "Chocolate Fondant",
    description: "Decadent dark chocolate cake with velvety cocoa dust",
    price: "$7.00",
    category: "desserts",
    imagePath: "src/assets/chocolate-cake.jpg",
    ingredients: ["Dark Chocolate", "Butter", "Eggs", "Sugar", "Flour", "Cocoa Powder"],
    calories: 420,
    dietaryTags: ["Vegetarian", "Contains Gluten"],
  },
  {
    _id: "menu-item-15",
    _type: "menuItem",
    name: "Fresh Croissant",
    description: "Buttery, flaky layers baked fresh every morning",
    price: "$4.50",
    category: "desserts",
    imagePath: "src/assets/croissant.jpg",
    ingredients: ["Flour", "Butter", "Yeast", "Sugar", "Salt"],
    calories: 280,
    dietaryTags: ["Vegetarian", "Contains Gluten", "Contains Dairy"],
  },
  {
    _id: "menu-item-16",
    _type: "menuItem",
    name: "Tiramisu",
    description: "Classic Italian layers of mascarpone and espresso",
    price: "$8.00",
    category: "desserts",
    ingredients: ["Mascarpone", "Espresso", "Ladyfingers", "Cocoa", "Eggs"],
    calories: 380,
    dietaryTags: ["Vegetarian", "Contains Gluten", "Contains Dairy"],
  },
  {
    _id: "menu-item-17",
    _type: "menuItem",
    name: "Cheesecake",
    description: "New York style with berry compote",
    price: "$7.50",
    category: "desserts",
    ingredients: ["Cream Cheese", "Graham Cracker", "Eggs", "Sugar", "Mixed Berries"],
    calories: 410,
    dietaryTags: ["Vegetarian", "Contains Gluten"],
  },
  {
    _id: "menu-item-18",
    _type: "menuItem",
    name: "Cinnamon Roll",
    description: "Warm, gooey, with cream cheese glaze",
    price: "$5.00",
    category: "desserts",
    ingredients: ["Flour", "Cinnamon", "Brown Sugar", "Butter", "Cream Cheese Glaze"],
    calories: 340,
    dietaryTags: ["Vegetarian", "Contains Gluten"],
  },
  {
    _id: "menu-item-19",
    _type: "menuItem",
    name: "Avocado Toast",
    description: "Smashed avocado on sourdough with chili flakes",
    price: "$9.00",
    category: "bites",
    ingredients: ["Sourdough Bread", "Avocado", "Chili Flakes", "Lime", "Sea Salt"],
    calories: 320,
    dietaryTags: ["Vegan", "Contains Gluten"],
  },
  {
    _id: "menu-item-20",
    _type: "menuItem",
    name: "Egg Benedict",
    description: "Poached eggs, hollandaise, on English muffin",
    price: "$12.00",
    category: "bites",
    ingredients: ["Eggs", "English Muffin", "Hollandaise Sauce", "Ham", "Chives"],
    calories: 480,
    dietaryTags: ["Contains Gluten", "Contains Dairy"],
  },
  {
    _id: "menu-item-21",
    _type: "menuItem",
    name: "Granola Bowl",
    description: "House-made granola with yogurt and seasonal fruits",
    price: "$8.00",
    category: "bites",
    ingredients: ["House Granola", "Greek Yogurt", "Seasonal Fruits", "Honey"],
    calories: 290,
    dietaryTags: ["Vegetarian", "Gluten-Free"],
  },
  {
    _id: "menu-item-22",
    _type: "menuItem",
    name: "Grilled Cheese",
    description: "Aged cheddar and gruyère on artisan bread",
    price: "$9.50",
    category: "bites",
    ingredients: ["Aged Cheddar", "Gruyère", "Artisan Sourdough", "Butter"],
    calories: 450,
    dietaryTags: ["Vegetarian", "Contains Gluten", "Contains Dairy"],
  },
  {
    _id: "menu-item-23",
    _type: "menuItem",
    name: "Caesar Salad",
    description: "Crisp romaine, parmesan, house-made croutons",
    price: "$10.00",
    category: "bites",
    ingredients: ["Romaine Lettuce", "Parmesan", "Croutons", "Caesar Dressing", "Anchovies"],
    calories: 260,
    dietaryTags: ["Contains Gluten", "Contains Dairy"],
  },
];

function toImportDocument(item) {
  const doc = {
    _id: item._id,
    _type: item._type,
    name: item.name,
    description: item.description,
    price: item.price,
    category: item.category,
    ingredients: item.ingredients,
    calories: item.calories,
    dietaryTags: item.dietaryTags,
  };

  if (item.videoUrl) {
    doc.videoUrl = item.videoUrl;
  }

  if (item.imagePath) {
    const absoluteImagePath = path.resolve(appDir, item.imagePath);
    doc.image = {
      _type: "image",
      _sanityAsset: `image@file://${absoluteImagePath}`,
    };
  }

  return doc;
}

fs.mkdirSync(outputDir, { recursive: true });
const ndjson = menuItems.map((item) => JSON.stringify(toImportDocument(item))).join("\n");
fs.writeFileSync(outputPath, `${ndjson}\n`, "utf8");

console.log(`Wrote ${menuItems.length} menu items to ${outputPath}`);
